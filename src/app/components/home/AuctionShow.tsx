  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-50" style={{zIndex: 45}}>
      ...
      {AuctionData &&
        ...
        JSON.parse(AuctionData.picture).map((_picture: string, index: number) => (
        ...
        {AuctionData ? (
          <>
            ...
            <div className="mb-4 overflow-auto h-1/3">
              ...
              {
                AuctionData.fields && JSON.parse(AuctionData.fields).map((field: any, i: number) => (
            ...
            {showAuctionPurposeSend && (
              <AuctionPurposeSend onClose={() => setShowAuctionPurposeSend(false)} Auction={AuctionData} AuctionId={AuctionData?.id || 0} />
            )}
          </>
        ) : (
