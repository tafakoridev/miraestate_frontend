...
const TenderPurposeSend: React.FC<TenderPurposeSendProps> = ({ onClose, tenderId, tender }) => {
...
useEffect(() => {
  ...
  const handleSendPurpose = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/tenders/purpose/send`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handlePay = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/tenders/pay/fee`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        ...
        <table className="table-auto border-collapse w-[1000px] text-center md:w-full">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="border text-blue-800 bg-slate-300">{Number((Option?.deposit_percentage/100) * tender.price)} تومان </td>
                </tr>
                <tr>
                ...
                <tr>
                  ...
                  <td className="border text-blue-800 bg-slate-300">{Number(Option.site_share) + Number((Option?.deposit_percentage/100) * tender.price)} تومان</td>
                </tr>
              </tbody>
            </table>
          }
...
export default TenderPurposeSend;
