...
function Navbar({ SetOpen, open }: any) {
  ...
  function calculateTotalCounts(countsObject: CountsObject) {
    ...
    const { tendersCount = 0, auctionsCount = 0, commoditiesCount = 0, tendersCountDecline = 0, auctionsCountDecline = 0, commoditiesCountDecline = 0, } = countsObject;
  
    // Calculate the sum
    const total = tendersCount + auctionsCount + commoditiesCount + tendersCountDecline + auctionsCountDecline + commoditiesCountDecline;
  ...
  const fetchAgentsInCounter = async (): Promise<void> => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/users/agents/in/counter`, {
        method: 'GET',
        headers: {
  ...
  return (
    <nav className={` ${open ? 'justify-end' : 'justify-between'} w-full h-full bg-gradient-to-l from-blue-900 via-blue-500 to-blue-900 shadow-md border-b-1 border-gray-100 flex items-center px-8 `}>
      {!open && (
        <button
      ...
      <div className="relative">
        ...
        >
          ...
          <span className="flex opacity-95 justify-center items-end w-[18px] h-[18px] aspect-square text-xs font-semibold leading-none text-white bg-red-600 rounded-full absolute right-0 bottom-0">{agentsCount}</span>
