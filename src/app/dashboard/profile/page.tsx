...
const Profile: React.FC = () => {
  ...
  const handleAddEducation = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/users/${user?.id}/educations`, {
        method: 'POST',
        headers: {
  ...
  const handleDeleteEmployee = async (employeeId: any) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/users/${user?.id}/employees/${employeeId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleDeleteEducation = async (educationId: any) => {
    const token = GetToken();
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/users/${user?.id}/educations/${educationId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleAddEmployee = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/users/${user?.id}/employees`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        ...
        <>
            ...
            <tbody>
              {user.educations.map((education) => (
                <tr key={education.id} className="border border-slate-300">
                  ...
                  <td className="border border-slate-300">{educationLevelMapping[education.educational_level as keyof typeof educationLevelMapping]}</td>
                  <td className="border border-slate-300">{education.field_of_study}</td>
                  <td className="border border-slate-300">{education.educational_institution}</td>
                  <td className="border border-slate-300">{moment(education.from).format('jYYYY-jM-jD')}</td>
                  <td className="border border-slate-300">{moment(education.to).format('jYYYY-jM-jD') ?? 'تاکنون'}</td>
                  <td className="border border-slate-300">{education.currently_enrolled ? 'بله' : 'خیر'}</td>
                  <td className="border border-slate-300">
                    <button onClick={() => handleDeleteEducation(education.id)} className="bg-red-500 text-white p-2 rounded-md hover:bg-red-700">
                      حذف
                    </button>
                  </td>
                </tr>
          ...
          <div className="mt-4">
            ...
            <form className="flex flex-row flex-wrap">
              ...
              <div className="mb-4 md:w-1/3 w-full px-3">
                <label htmlFor="educationalLevel" className="block text-sm font-medium text-gray-600">
                  سطح تحصیلات
                </label>
                <select
                  id="educationalLevel"
                  ...
                  onChange={(e) => setNewEducation({ ...newEducation, educational_level: e.target.value })}
                  className="mt-1 p-2 w-full border rounded-md"
                >
                  <option value="">انتخاب سطح تحصیلات</option>
                  {Object.keys(educationLevelMapping).map((level) => (
                    <option key={level} value={level}>
                      {educationLevelMapping[level as keyof typeof educationLevelMapping]}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="mb-4 md:w-1/3 w-full px-3">
                <label htmlFor="fieldOfStudy" className="block text-sm font-medium text-gray-600">
                  رشته تحصیلی
                </label>
                <input
                  type="text"
              ...
              <div className="mb-4 md:w-1/3 w-full px-3">
                <label htmlFor="institution" className="block text-sm font-medium text-gray-600">
                  مؤسسه آموزشی
                </label>
                <input
                  type="text"
                  ...
                  onChange={(e) => setNewEducation({ ...newEducation, educational_institution: e.target.value })}
                  className="mt-1 p-2 w-full border rounded-md"
                />
              </div>
              ...
              <div className="mb-4 md:w-1/3 w-full px-3">
                ...
                <DatePicker className="mt-1 p-2 w-full border rounded-md" onChange={(value: any) => setNewEducation({ ...newEducation, from: moment(value, 'jYYYY/jM/jD HH:mm').format('YYYY-M-D') })}
                  value={newEducation.from} dateRenge={{ end: "1420", start: "1371" }} />
              </div>
              ...
              <div className="mb-4 md:w-1/3 w-full px-3">
                ...
                <DatePicker className="mt-1 p-2 w-full border rounded-md" onChange={(value: any) => setNewEducation({ ...newEducation, to: moment(value, 'jYYYY/jM/jD HH:mm').format('YYYY-M-D') })}
                  value={newEducation.to} dateRenge={{ end: "1420", start: "1371" }} />
              </div>

              <div className="mb-4 md:w-1/3 w-full px-3">
                <label htmlFor="currentlyEnrolled" className="block text-sm font-medium text-gray-600">
                  در حال تحصیل
                </label>
                <select
                  ...
                  onChange={(e) => setNewEducation({ ...newEducation, currently_enrolled: e.target.value === 'true' })}
                  className="mt-1 p-2 w-full border rounded-md"
                >
            ...
            <tbody>
              {user.employees.map((employee) => (
                <tr key={employee.id} className="border border-slate-300">
                  ...
                  <td className="border border-slate-300">{employee.company_name}</td>
                  <td className="border border-slate-300">{employee.job_title}</td>
                  <td className="border border-slate-300">{moment(employee.from).format('jYYYY-jM-jD')}</td>
                  <td className="border border-slate-300">{moment(employee.to).format('jYYYY-jM-jD') ?? 'تاکنون'}</td>
                  <td className="border border-slate-300">{employee.currently_enrolled ? 'بله' : 'خیر'}</td>
                  <td className="border border-slate-300">
                    <button onClick={() => handleDeleteEmployee(employee.id)} className="bg-red-500 text-white p-2 rounded-md hover:bg-red-700">
                      حذف
                    </button>
                  </td>
                </tr>
          ...
          <div className="mt-4">
            ...
            <form className="flex flex-row flex-wrap">
              ...
              <div className="mb-4 md:w-1/3 w-full px-3">
                ...
                <DatePicker
                  className="mt-1 p-2 w-full border rounded-md"
                  onChange={(value: any) => setNewEmployee({ ...newEmployee, from: moment(value, 'jYYYY/jM/jD HH:mm').format('YYYY-M-D') })}
              ...
              <div className="mb-4 md:w-1/3 w-full px-3">
                ...
                <DatePicker
                  className="mt-1 p-2 w-full border rounded-md"
                  onChange={(value: any) => setNewEmployee({ ...newEmployee, to: moment(value, 'jYYYY/jM/jD HH:mm').format('YYYY-M-D') })}
              ...
              <div className="mb-4 md:w-1/3 w-full px-3">
                ...
                <select
                  ...
                  onChange={(e) => setNewEmployee({ ...newEmployee, currently_enrolled: e.target.value === 'true' })}
                  className="mt-1 p-2 w-full border rounded-md"
                >
                  <option value="true">بله</option>
                  <option value="false">خیر</option>
