...
const Home: React.FC = () => {
  ...
  const fetchCommodities = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/commodities/bycity/list/${city_id}`, {
        headers: {
        ...
        <div className="flex w-full gap-5 flex-wrap justify-center overflow-auto">
          {
            commodities.map((el, i) => (
              <Kala id={el.id} select={async (id: any) => setCommodity_id(id)} key={i} title={el.title} price={el.price} imageSrc={el.picture} />
            ))
          }
